...
class LevMarOptimizer(Optimizer):
    ...
    def __init__(self, linesearchmethod: LineSearch, maxiterations = 15, initial_lam = 1, reduction_lam=1, epsilon=np.sqrt(np.finfo(np.float).eps), minreduction=1e-4,max_error_ratio=(0.05,0.95), differencing=Optimizer.Differencing.forward):
